  ...
  const handleMicrosoftLogin = async () => {
    try {
      ...
      if (!userEmail.endsWith('@student.gsu.edu') && !userEmail.endsWith('@gsu.edu')) {
        ...
        alert('Please sign in with your GSU email address (@student.gsu.edu or @gsu.edu)')
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      if (!isGoogleReady) {
        ...
        alert('Google authentication is not ready. Please try again.')
        return
      }
      ...
      // For Google login, we'll ask them to provide their GSU email separately
      // since they're signing in with their personal Google account
      const gsuEmail = prompt(
        'Please enter your GSU student email address (@student.gsu.edu):\n\n' +
        'Note: You signed in with your personal Google account, but we need your GSU email for course access.'
      )
      
      if (!gsuEmail) {
        console.log('User cancelled GSU email entry')
        return
      }
      
      // Validate GSU email format
      if (!gsuEmail.endsWith('@student.gsu.edu') && !gsuEmail.endsWith('@gsu.edu')) {
        console.error('Invalid GSU email format')
        alert('Please enter a valid GSU email address (@student.gsu.edu or @gsu.edu)')
        return
      }
      ...
      sessionStorage.setItem('gsuEmail', gsuEmail)
